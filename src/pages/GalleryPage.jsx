    ...
    <section className="space-y-6">
      ...
      <div className="mt-6 grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-4">
        {IMAGE_URLS.map((image, index) => (
          <ScrollReveal key={image} delay={(index % 8) * 60}>
            <img
              src={image}
              alt={`Louisline travel gallery ${index + 1}`}
              className="h-40 w-full rounded-xl border border-slate-200 object-cover shadow-lg transition duration-300 hover:-translate-y-1 hover:scale-[1.02] hover:shadow-2xl md:h-48"
            />
          </ScrollReveal>
        ))}
      </div>
